...
Photo_ave_select <- select(Sleep_stage, Time, DeltaF, Fluo)
...
Photo_ave_select_agg <- aggregate(Photo_ave_select[, 2:ncol_stg], list(Photo_ave_select$Downsampleg), var)

## writing to csv ##
write.csv(Photo_ave_select_agg, file = file.choose(TRUE))   
