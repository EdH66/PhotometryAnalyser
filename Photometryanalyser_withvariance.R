#Photometry analyser script with additional variance output. Use to prepare photometry data exported from spike for plotting.
#Export from spike as spreadsheet text at 1Hz. Open file and remove channel number from column names. e.g "1 Delta(701)" becomes "Delta".
#Run photometry_variance_extract10x script first on 10 Hz data and append results as DeltaVAR and FluoVar into input file csv.
#Script constructs data frame for scatter plots in origin
#Calculates exact length of each sleep episode from sleep scored hypno channel. Hypno script must ahve been run in Spike2.
#For data per episode all data (inc variance) is averaged over the episode length. 
...
#For episode lengths data will be averaged. Average variance is better than average value (see variance script).
...
## now sort Fluorescence by grouping variable g and aggregating other other variables (delta, emg e.t.c) into episode bins. 

# aggregate by variance # don't aggregate variance data ##
ncol_stg = ncol(Sleep_stage_group)-1
Sleep_episode_aggvar <- aggregate(Sleep_stage_group[, 2:ncol_stg], list(Sleep_stage_group$g), mean)
...
##CURRENTLY TAKING VARIANCE OF THE DELTA F VAR AND FLUO VAR - should average and recombine with
...
c <- ggplot(Sleep_stage, aes(x=Ep_length, y=FluoVAR, color= as.factor(State))) +
  geom_point() +
  scale_y_log10() + 
  scale_x_continuous(trans='log10') +
  xlab("EMG") +
...
d <- ggplot(Sleep_vardistbyepisode, aes(x=Ep_length, y=FluoVAR, color= as.factor(State))) +
  geom_point() +
  scale_y_log10() + 
  scale_x_continuous(trans='log10') +
  xlab("Episode length") +
